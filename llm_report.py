...
def _build_prompt(timeframes_config, all_timeframes):
    ...
    system_prompt = (
        ...
        "Factors that determine SURF SIZE (the wave height at the break):\n"
        "- Effective height (metres face): the actual wave size at the beach, after "
        "headland diffraction and shoaling. small <1m, moderate 1-2m, big 2-3m, huge >3m.\n"
        "- Exposure (%): how much of the offshore swell energy reaches this beach. "
        "100% = fully exposed (direct swell window). Lower = shadowed by headlands.\n"
        "- Wave period: <8s = weak windswell (little push), 8-12s = moderate, "
        ">12s = powerful groundswell (long-period waves carry more energy).\n\n"
        "Factors that determine SURF QUALITY (how rideable the waves are):\n"
        "- Embayment factor (0-1): how open the beach is for this swell size. "
        "<0.50 = close-out risk (beach too narrow for the wave), 0.50-0.75 = marginal, "
        ">=0.75 = plenty of lateral room, A-frame potential.\n"
        "- Attack angle: swell direction vs beach face. 15-45\u00b0 = perfect peeling waves. "
        "Near 0\u00b0 = close-out (straight on). Over 60\u00b0 = weak (extreme angle).\n"
        "- Wind relative to beach: offshore (from land) = clean face; "
        "onshore (from sea) = choppy/messy; cross = neutral.\n"
        "- Tide factor (0.6-1.0): 1.0 at optimal 0.5-1.5m tide range; drops at extremes.\n"
        "- Overall quality = wind \u00d7 attack \u00d7 tide \u00d7 embayment (product).\n"
        "  A low value in any one factor drags the whole quality down.\n\n"
