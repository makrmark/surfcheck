...
def _build_prompt(timeframes_config, all_timeframes):
    ...
    system_prompt = (
        ...
        "swell size.\n"
        "- Beach slope: flat beaches (Dee Why, Long Reef) don't amplify waves much; steep beaches (North/South Steyne) jack waves up.\n"
        "- Breaker quality: even if a wave reaches the beach, it may not break cleanly. Small waves on a flat beach at low tide just surge.\n"
    ...
    for i, tf_conf in enumerate(timeframes_config):
        ...
        for bc in tf_data["beach_conditions"]:
            user_prompt += (
                ...
                f"slope {bc.get('slope', 'moderate')}, "
                f"breaker {bc.get('breaker_factor', 1.0):.0%}, "
